...
CunicodeDlg::CunicodeDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CunicodeDlg::IDD, pParent)
{
...
void CunicodeDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT4, undo);
}

BEGIN_MESSAGE_MAP(CunicodeDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON1, &CunicodeDlg::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &CunicodeDlg::OnBnClickedButton2)
END_MESSAGE_MAP()
...
{
	m_first.GetWindowText(first);
	m_second.GetWindowText(second);
	
	result = wchar_t(first[0] + second[0]);

	m_result.SetWindowText(result);
}


void CunicodeDlg::OnBnClickedButton2()
{
	m_result.GetWindowText(first);
	m_second.GetWindowText(second);

	result = wchar_t(first[0] - second[0]);

	undo.SetWindowText(result);
}
